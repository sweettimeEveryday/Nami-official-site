...
(function () {
  ...
  /* 流れるエリアの画像。PCは assets/img/ の 800px、スマホは
     assets/img/sp/ の 560px。sp が無ければ自動で PC 用に戻す。 */
  ...
  var lbAll = document.getElementById('lbAll');
  var last = null;
  ...
  /* ---- 6. パネルの中で隣の一枚へ ----
     31枚を HTML に書いた順で並べ、矢印・横スワイプ・←→キーで移る。
     何度も帯を押し直さなくて済むように。 */
  var at = -1;            /* いま出している items の位置 */
  var drafts = {};        /* 書きかけの感想。作品ごとに覚えておく */

  /* パネル用の大きい画像。流れるエリアは 800px（スマホ 560px）だが、
     パネルは 4:3 の枠を 520px 幅で描くので、2倍・3倍の画面では
     1030〜1040px 必要になる。そのぶんだけを、パネルを開いたときに読む。
     ページを開いた時点では1バイトも取りに行かない。
     置いていない絵は流れるエリアの1枚のまま（読めなければ何も起きない）。 */
  var BIG_DIR = 'assets/img/big/';

  function bigPath(path) {
    if (!BIG_DIR) { return ''; }
    var m = path.match(/([^\/]+)\.[a-z0-9]+$/i);
    return m ? BIG_DIR + m[1] + '.webp' : '';
  }

  /* 読み終えた大きい画像を覚えておく。行ったり来たりしても取り直さない */
  var bigDone = {};
  function warmBig(i) {
    if (!BIG_DIR || !items.length) { return; }
    var it = items[((i % items.length) + items.length) % items.length];
    var b = bigPath(it.getAttribute('data-img'));
    if (!b || bigDone[b]) { return; }
    var im = new Image();
    im.decoding = 'async';
    im.onload = function () { bigDone[b] = 1; };
    im.src = b;
  }

  /* 写真を入れ替える。まず軽い1枚（キャッシュ済み）を即座に出し、
     大きい版があれば読み終えてから静かに差し替える。 */
  var imgTurn = 0;
  function setPhoto(item) {
    var turn = ++imgTurn;
    var path = item.getAttribute('data-img');
    lbImg.onerror = function () { this.onerror = null; this.src = path; };
    lbImg.src = light(path);
    lbImg.alt = item.getAttribute('data-title') || '';
    /* パネルの写真枠は 4:3。縦長の絵をそのまま入れると真ん中＝
       腰のあたりが出て顔が切れるので、カードと同じ data-pos を効かせる。 */
    var pos = (item.getAttribute('data-pos') || '50').trim().split(/\s+/);
    lbImg.style.objectPosition = pos.length > 1
      ? pos[0] + '% ' + pos[1] + '%'
      : 'center ' + pos[0] + '%';

    var big = bigPath(path);
    if (!big) { return; }
    var pre = new Image();
    pre.onload = function () {
      bigDone[big] = 1;
      /* 待っているあいだに次の一枚へ移っていたら、もう差し替えない */
      if (turn === imgTurn) { lbImg.src = big; }
    };
    pre.src = big;
  }
  ...
  function show(i, how) {
    if (!items.length) { return; }
    /* 書きかけを預かってから移る */
    if (at >= 0 && cta && !cta.hidden) { drafts[work] = ctaBox.value; }

    at = ((i % items.length) + items.length) % items.length;
    var item = items[at];

    setPhoto(item);
    if (lbNow) { lbNow.textContent = at + 1; }
    ...
    if (ask) {
      ...
      ctaBox.value = drafts[work] || '';
      wrote = ctaBox.value !== '';
      state = wrote ? 'draft' : 'shown';
      draw();
    } else {
    ...
    /* 本文が長い一枚から短い一枚へ移ったとき、スクロール位置が残らないように */
    var body = lb.querySelector('.lb__body');
    if (body) { body.scrollTop = 0; }

    /* 隣の一枚を先に読んでおく。矢印を押した瞬間から鮮明にするため。
       いま出している1枚の表示を邪魔しないよう、少し遅らせる。 */
    clearTimeout(warmT);
    warmT = setTimeout(function () { warmBig(at + 1); warmBig(at - 1); }, 500);

    track('panel_view', { work_id: work, has_link: href ? 1 : 0, via: how || 'card' });
    if (ask) { track('cta_shown', { work_id: work, ask_variant: variant }); }
  }
  var warmT = null;

  function step(d, how) { show(at + d, how); }

  if (lbPrev) { lbPrev.addEventListener('click', function () { step(-1, 'arrow'); }); }
  if (lbNext) { lbNext.addEventListener('click', function () { step(1, 'arrow'); }); }

  /* 横スワイプ。縦に指を動かしたときは何もしない（誤爆を防ぐ） */
  if (lbFig) {
    ...
    lbFig.addEventListener('pointercancel', function () { on = false; });
  }
  ...
  function open(item) {
    ...
    drafts = {};
    at = -1;
    if (lbAll) { lbAll.textContent = items.length; }
    show(items.indexOf(item), 'card');
  ...
  document.addEventListener('keydown', function (e) {
    ...
    /* 感想を書いている最中の ←→ は文字の移動。奪わない */
    if (cta && document.activeElement === ctaBox) { return; }
...
(function () {
  ...
  Array.prototype.forEach.call(document.querySelectorAll('[data-sarea]'), function (box) {
    ...
    /* ---- 末尾まで読んだら、その先はページへ渡す ----
       CSS の overscroll-behavior:auto だけでは足りない。ブラウザは
       「いま回しはじめた枠」にホイールを結びつけたままにするので
       （スクロールラッチ）、枠が末尾に着いても、指を止めていったん
       操作を切るまでページが動かない。ここで自分で渡してしまう。
       先頭でも同じで、上へ戻りきったらページが上へ動く。 */
    var atEnd = function (d) {
      var max = vp.scrollHeight - vp.clientHeight;
      if (max <= 0) { return true; }           /* 中身が短くて動かない枠 */
      return d > 0 ? (vp.scrollTop >= max - 1) : (vp.scrollTop <= 0);
    };
    /* ページを d ピクセルだけ動かす。style.css の html{scroll-behavior:smooth}
       が効いたままだと1回ごとに滑らかな移動が始まってしまい、指の動きに
       ついてこない。ここだけ一時的に外して、動かしたら戻す。 */
    var pageBy = function (d) {
      var el = document.documentElement, keep = el.style.scrollBehavior;
    ...
    vp.addEventListener('wheel', function (e) {
      if (e.ctrlKey || e.deltaY === 0) { return; }   /* 拡大縮小には触らない */
      if (!atEnd(e.deltaY)) { return; }
      /* deltaMode: 0=ピクセル 1=行 2=ページ。行・ページで来る環境がある */
      var unit = e.deltaMode === 1 ? 16 : (e.deltaMode === 2 ? window.innerHeight : 1);
      e.preventDefault();
      pageBy(e.deltaY * unit);
    }, { passive: false });
    ...
    vp.addEventListener('touchstart', function (e) {
      ...
      if (on) { ty = e.touches[0].clientY; }
    }, { passive: true });
    vp.addEventListener('touchmove', function (e) {
      if (!on || e.touches.length !== 1) { return; }
      var y = e.touches[0].clientY;
      var dy = ty - y;                       /* 下へ送る指づかい＝dy>0 */
      if (dy === 0) { return; }
      if (!atEnd(dy)) { ty = y; return; }
      e.preventDefault();
      pageBy(dy);
      ty = y;
    }, { passive: false });
    vp.addEventListener('touchend', function () { on = false; }, { passive: true });
  });
})();
